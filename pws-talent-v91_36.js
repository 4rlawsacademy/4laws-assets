  ...
  */
  ...
  (function pwsArmoryCss_() {
    ...
    st.appendChild(document.createTextNode(
      ...
      '#pwsBrunoDock.folded .pws-unlock-panel > *:not(.pws-unlock-header){display:none !important;}' +
      '#pwsBrunoDock.folded .pws-unlock-header{cursor:pointer;}' +
  ...
  window.pwsBrunoOpen_ = function pwsBrunoOpen_(seed) {
    var dock = document.getElementById('pwsBrunoDock'); if (dock) dock.classList.remove('folded');
    if (!pwsBrunoIs_()) {
    ...
    try { if (dock) dock.scrollIntoView({ block: 'start', behavior: 'smooth' }); } catch (eV) {}
    return pwsBrunoIs_();
  };
  ...
  (function wireBrunoDock_() {
    function tryWire() {
      ...
      dock.addEventListener('click', function(e) {
        ...
        var hdr = e.target.closest ? e.target.closest('.pws-unlock-header') : null;
        if (hdr && !(e.target.closest('button'))) { pwsBrunoFold_(false); }
      });
    }
    tryWire();
  })();
  window.pwsBrunoBoot_ = function pwsBrunoBoot_() { try { if (!_useTool && _session && _memberId) { pwsBrunoOpen_(); } } catch (eB) {} };
  ...
  function pwsRenderToolsTile() {
    ...
    var newToolBtn = document.createElement('button'); newToolBtn.type = 'button'; newToolBtn.className = 'pws-armory-primary';
    newToolBtn.textContent = _lang === 'es' ? '+ NUEVA HERRAMIENTA' : '+ NEW ACTIVITY TOOL';
    newToolBtn.addEventListener('click', function() { pwsBrunoOpen_(); });
    armoryDoors.appendChild(newToolBtn);
    var arsenalA = document.createElement('a'); arsenalA.href = '/arsenal'; arsenalA.textContent = '\u2694 ' + (_lang === 'es' ? 'EL ARSENAL' : 'THE ARSENAL');
    armoryDoors.appendChild(arsenalA);
  ...
  window.pwsCloseUse = function pwsCloseUse() {
    ...
    if (!_useSwitching) { setTimeout(function() { try { pwsBrunoOpen_(); } catch (eRe) {} }, 0); }
    void wasBruno;
  };
