...
(function() {
  ...
  */
  ...
  function pwsRenderToolsTile() {
    ...
    stageRow.appendChild(stageInp); stageRow.appendChild(stageMic); stageRow.appendChild(stageClip); stageRow.appendChild(stageCut);
  ...
  function pwsRenderUseFooter(isTimer, isMedia, savedLink) {
    ...
    flagWrap.addEventListener('drop', function(eF) {
      eF.preventDefault();
      flagImg.style.boxShadow = '0 4px 16px rgba(0,0,0,0.55), 0 0 12px rgba(200,168,75,0.22)';
      if (eF.dataTransfer && eF.dataTransfer.files && eF.dataTransfer.files[0] && typeof DocBCore !== 'undefined') {
        DocBCore.handleCameraFile(eF.dataTransfer.files[0]);
        var inpF = document.getElementById('pwsUseInput');
        if (inpF) { try { inpF.focus(); } catch (eFo) {} }
      }
    });
    footer.appendChild(flagWrap);
