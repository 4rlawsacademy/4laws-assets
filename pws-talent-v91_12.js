/* BENCH 29 pws-talent v91_12 — THE WEAPONS SHELF (founder's loop, 8/27:
...
(function() {
  ...
  */
  ...
  /* v91_12 THE WEAPONS SHELF (founder's loop, 8/27): every weapon Ace
     ships to the /arsenal wall now also appears here, in the member's own
     Tools & Entertainment -- the library card. Reads the same wall door
     the /arsenal page reads (armoryGetRequests, shipped + linked only),
     cached once per page load. Each weapon: image, title, enemy line,
     open hand, and the copy hand (clipboard + old-browser fallback) so
     equipping any activity or todo is copy -> paste into its Links. */
  var _pwsArsenalCache = null;
  function pwsArsenalCopy_(btn, link) {
    function ok() {
      var t = btn.innerHTML;
      btn.innerHTML = (_lang === 'es') ? '\u2713 Copiado' : '\u2713 Copied';
    ...
    function fb() {
      ...
      try { document.execCommand('copy'); ok(); } catch (eC) {}
      document.body.removeChild(ta);
    }
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard.writeText(link).then(ok).catch(fb);
    } else { fb(); }
  }
  window.pwsArsenalCopy_ = pwsArsenalCopy_;
  function pwsArsenalFill_(body, weapons) {
    var es = (_lang === 'es');
    body.innerHTML = '';
    if (!weapons.length) {
      body.innerHTML = '<div style="font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(240,230,204,0.45);font-size:17px !important;">' +
        (es ? 'Las primeras armas est\u00e1n en la fragua.' : 'The first weapons are at the forge.') + '</div>';
    } else {
      weapons.forEach(function(w) {
        var card = document.createElement('div');
        card.style.cssText = 'display:flex;gap:12px;align-items:center;padding:10px 0;border-bottom:1px solid rgba(200,168,75,0.15);';
        var h = '';
        if (w.imageUrl) {
          h += '<img src="' + escHtml(w.imageUrl) + '" alt="" onerror="this.style.display=\u0027none\u0027" style="width:64px;height:64px;object-fit:cover;border-radius:6px;border:1px solid rgba(200,168,75,0.4);flex-shrink:0;">';
        } else {
          h += '<div style="width:64px;height:64px;border-radius:6px;border:1px solid rgba(200,168,75,0.4);display:flex;align-items:center;justify-content:center;font-size:26px;flex-shrink:0;">\u2694</div>';
        }
        var desc = (w.description || '');
        if (desc.length > 90) desc = desc.substring(0, 90) + '\u2026';
        h += '<div style="flex:1;min-width:0;">' +
          '<div style="font-family:Cinzel,serif;font-size:17px !important;color:#f0e6cc;letter-spacing:0.06em;">' + escHtml(w.title || '') + '</div>' +
          (desc ? '<div style="font-family:Cormorant Garamond,serif;font-style:italic;font-size:16px !important;color:rgba(220,80,80,0.85);">' + escHtml(desc) + '</div>' : '') +
          '<div style="display:flex;gap:10px;margin-top:6px;flex-wrap:wrap;">' +
            '<a href="' + escHtml(w.shippedLink) + '" target="_blank" rel="noopener" style="font-family:Cinzel,serif;font-size:13px !important;letter-spacing:0.1em;color:#c8a84b;border:1px solid rgba(200,168,75,0.5);border-radius:4px;padding:6px 12px;text-decoration:none;">\u2694 ' + (es ? 'Abrir' : 'Open') + '</a>' +
            '<button type="button" onclick="pwsArsenalCopy_(this,' + JSON.stringify(w.shippedLink).replace(/"/g, '&quot;') + ')" style="font-family:Cinzel,serif;font-size:13px !important;letter-spacing:0.1em;color:#c8a84b;background:transparent;border:1px solid rgba(200,168,75,0.5);border-radius:4px;padding:6px 12px;cursor:pointer;">\uD83D\uDD17 ' + (es ? 'Copiar enlace' : 'Copy link') + '</button>' +
          '</div></div>';
        card.innerHTML = h;
        body.appendChild(card);
      });
    }
    var road = document.createElement('a');
    road.href = '/arsenal';
    road.style.cssText = 'display:inline-block;margin-top:10px;font-family:Cormorant Garamond,serif;font-style:italic;font-size:17px !important;color:rgba(200,168,75,0.8);text-decoration:none;';
    road.innerHTML = es ? '\u2192 Ver la pared completa' : '\u2192 See the full wall';
    body.appendChild(road);
  }
  function pwsRenderArsenalShelf_(container) {
    var es = (_lang === 'es');
    var shelf = document.createElement('div');
    shelf.id = 'pwsArsenalShelf';
    shelf.style.cssText = 'margin:22px 0 18px 0;padding:16px;border:1px solid rgba(200,168,75,0.35);border-radius:8px;background:rgba(200,168,75,0.04);';
    shelf.innerHTML = '<div style="font-family:Cinzel,serif;font-size:18px !important;letter-spacing:0.22em;color:#c8a84b;text-transform:uppercase;margin-bottom:4px;">\u2694 ' + (es ? 'EL ARSENAL' : 'THE ARSENAL') + '</div>' +
      '<div style="font-family:Cormorant Garamond,serif;font-size:17px !important;font-style:italic;color:rgba(240,230,204,0.55);margin-bottom:12px;">' +
      (es ? 'Armas forjadas por Ace \u2014 copia el enlace y equ\u00edpala en cualquier actividad o tarea.' : 'Weapons forged by Ace \u2014 copy a link and equip it onto any activity or todo.') + '</div>';
    var body = document.createElement('div');
    body.id = 'pwsArsenalBody';
    body.innerHTML = '<div style="font-family:Cormorant Garamond,serif;font-style:italic;color:rgba(240,230,204,0.4);font-size:16px !important;">' + (es ? 'Leyendo la pared\u2026' : 'Reading the wall\u2026') + '</div>';
    shelf.appendChild(body);
    container.appendChild(shelf);
    if (_pwsArsenalCache) { pwsArsenalFill_(body, _pwsArsenalCache); return; }
    post({ action: 'armoryGetRequests' }).then(function(d) {
      if (!d || d.status !== 'ok') { pwsArsenalFill_(body, []); return; }
      _pwsArsenalCache = (d.requests || []).filter(function(r) { return r.status === 'shipped' && r.shippedLink; });
      pwsArsenalFill_(body, _pwsArsenalCache);
    }).catch(function() { pwsArsenalFill_(body, []); });
  }
  ...
  function pwsRenderToolsTile() {
    ...
    pwsRenderArsenalShelf_(container); /* v91_12: the weapons shelf, after the member's own tools */
