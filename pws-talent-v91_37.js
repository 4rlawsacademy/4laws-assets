  ...
  */
  ...
  (function pwsArmoryCss_() {
    ...
    st.appendChild(document.createTextNode(
      ...
      '#pwsBrunoCard{display:none;position:relative;min-height:150px;border:1px solid rgba(200,168,75,0.45);border-radius:10px;overflow:hidden;cursor:pointer;background:linear-gradient(135deg,#0d1117 0%,#1a1408 100%);background-size:cover;background-position:center;box-shadow:0 8px 32px rgba(0,0,0,0.5);}' +
  ...
  function pwsBrunoGreeting_() {
    ...
    if (_lang === 'es') {
      return '\u00a1Bruno Herrera, a tus \u00f3rdenes! Maestro herrero de esta Academia. Veo ' + n + (n === 1 ? ' herramienta' : ' herramientas') + ' en tu pared' + (un ? (', ' + un + ' sin activar todav\u00eda') : '') + '. Mu\u00e9strame la trampa \u2014 \u00bfqu\u00e9 trabajo odias, temes o sigues evitando?';
    }
    return '\u00a1Bruno Herrera, at your service! Master toolmaker of this Academy. I see ' + n + (n === 1 ? ' tool' : ' tools') + ' on your wall' + (un ? (', ' + un + ' not yet activated') : '') + '. Show me the trap \u2014 what job do you hate, fear, or keep avoiding?';
  }
  ...
  window.pwsBrunoBoot_ = function pwsBrunoBoot_() { try { if (!_useTool && _session && _memberId) { pwsBrunoOpen_('', true); } } catch (eB) {} };   /* v91_37: loaded, but closed -- a card */
