  ...
  */
  ...
  (function pwsArmoryCss_() {
    ...
    st.appendChild(document.createTextNode(
      ...
      '.pws-tool-card.pws-weapon-card{position:relative;min-height:170px;padding:0 !important;overflow:hidden;cursor:pointer;background:#0d1117 center/cover no-repeat;border:1px solid rgba(200,168,75,0.45) !important;}' +
  ...
  function pwsBeltPropose_() {
    ...
    /* the weapon for this week's monster */
    try { var week = (_armory && _armory.week) || []; for (var w = 0; w < week.length; w++) { take(pwsWeaponFor_(week[w].key)); } } catch (e2) {}
    /* forged tools, then the Talent tools, then any other activated weapon */
    for (var f = 0; f < tools.length; f++) { if (String(tools[f].type || '') === 'forge') take(tools[f]); }
    for (var g = 0; g < tools.length; g++) { var tg = tools[g]; if (tg.isTalent === true || String(tg.type || '') === 'talent' || tg.isMastery === true) take(tg); }
    for (var k2 = 0; k2 < tools.length; k2++) { if (tools[k2].configured === true) take(tools[k2]); }
    for (var k3 = 0; k3 < tools.length; k3++) { take(tools[k3]); }
  ...
  function pwsIsRoutine_(t) {
    ...
    return !!(onDay[pwsToolKey_(t)] || ty === 'obligation' || ty === 'window' || ty === 'todo');
  }
  function pwsHangShelves_(container) {
    ...
    var tools = (_toolsData && _toolsData.confirmedTools) ? _toolsData.confirmedTools : [];
    var byKey = { pile: [], fog: [], cold: [], conq: [] }, es = _lang === 'es', imgs = window.PWS_ARMORY_IMAGES || {};
    /* the member's own tools that are weapons -- on every shelf they fight on */
    cards.forEach(function(c) {
      var idx = parseInt(c.getAttribute('data-tool-idx'), 10), t = isNaN(idx) ? null : tools[idx]; if (!t) return;
      if (pwsIsRoutine_(t)) { c.parentNode && c.parentNode.removeChild(c); return; }   /* the routine lives on Favorite Day */
      var fights = pwsFightsOf_(pwsToolKey_(t), t.law || t.lawTag || (t.isTalent ? 'talent' : ''));
      fights.forEach(function(k, i) { byKey[k].push(i === 0 ? c : c.cloneNode(true)); });
      if (fights.length > 1) { /* the clones need the same door */ }
    ...
    pageWeapons.forEach(function(w) {
      ...
      fights.forEach(function(k) {
        var wc = document.createElement('div'); wc.className = 'pws-tool-card pws-weapon-card';
    ...
    function openShelf(k) {
      ...
      if (!byKey[k].length) { var em = document.createElement('div'); em.className = 'pws-shelf-empty'; em.textContent = es ? 'Nada apunta a este monstruo todav\u00eda. Bruno puede forjar algo.' : 'Nothing aims at this monster yet. Bruno can forge one.'; body.appendChild(em); }
      byKey[k].forEach(function(c) { body.appendChild(c); });
    ...
    order.forEach(function(k) {
      ...
      m.addEventListener('click', function() { openShelf(_shelfOpen === k ? '' : k); });
      rowEl.appendChild(m);
    });
    container.appendChild(rowEl); container.appendChild(openEl);
    ...
    if (_shelfOpen) openShelf(_shelfOpen);
  }
