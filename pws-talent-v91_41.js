  ...
  */
  ...
  (function pwsArmoryCss_() {
    ...
    st.appendChild(document.createTextNode(
      ...
      '#pwsBrunoCard .pws-armory-card-title{position:absolute;left:0;right:0;bottom:0;padding:14px 18px;font-family:Cinzel,serif;font-size:22px;letter-spacing:0.18em;color:#c8a84b;text-shadow:0 2px 8px rgba(0,0,0,0.9);background:linear-gradient(transparent,rgba(4,6,8,0.85));}' +
      ...
      '#pwsBeltRow .pws-belt-tile{flex:1 1 120px;min-width:120px;max-width:100%;padding:10px 12px;border-radius:8px;background:rgba(240,230,204,0.05);border:1px solid rgba(240,230,204,0.18);color:#f0e6cc;font-family:\'Cormorant Garamond\',serif;font-size:16px;line-height:1.15;cursor:pointer;text-align:left;}' +
      ...
      '.pws-tool-card.pws-weapon-card{position:relative;min-height:110px;padding:0 !important;overflow:hidden;cursor:pointer;background:#0d1117 center/cover no-repeat;border:1px solid rgba(200,168,75,0.45) !important;}' +
      '.pws-tool-card.pws-weapon-card .pws-armory-card-title{position:absolute;left:0;right:0;bottom:0;padding:8px 12px;font-family:Cinzel,serif;font-size:13px;letter-spacing:0.12em;color:#c8a84b;text-shadow:0 2px 8px rgba(0,0,0,0.9);background:linear-gradient(transparent,rgba(4,6,8,0.9));}' +
      ...
      '.pws-shelf{grid-column:1 / -1;border:1px solid rgba(200,168,75,0.35);border-radius:10px;overflow:hidden;margin:4px 0;}' +
      '.pws-shelf-head{position:relative;min-height:110px;cursor:pointer;background:linear-gradient(135deg,#0d1117 0%,#1a1408 100%);background-size:cover;background-position:left center;}' +
      '.pws-shelf.open .pws-shelf-head{min-height:220px;}' +
      '.pws-shelf-title{position:absolute;left:0;right:0;bottom:0;padding:10px 16px;font-family:Cinzel,serif;font-size:18px;letter-spacing:0.16em;color:#c8a84b;text-shadow:0 2px 8px rgba(0,0,0,0.9);background:linear-gradient(transparent,rgba(4,6,8,0.88));display:flex;justify-content:space-between;align-items:baseline;gap:10px;}' +
      '.pws-shelf-heads{font-size:12px;letter-spacing:0.14em;color:rgba(240,230,204,0.75);white-space:nowrap;}' +
      '.pws-shelf-count{position:absolute;right:14px;top:10px;font-family:Cinzel,serif;font-size:11px;letter-spacing:0.14em;color:rgba(240,230,204,0.6);text-shadow:0 2px 6px rgba(0,0,0,0.9);}' +
      '.pws-shelf-body{display:none;padding:10px;gap:10px;grid-template-columns:repeat(2,minmax(0,1fr));}' +
      '.pws-shelf.open .pws-shelf-body{display:grid;}' +
  ...
  function pwsMonsterReading_() {
    ...
    function line(m, first) {
      ...
      var aw = null; for (var ai = 0; ai < _arsenal.length; ai++) { if (_arsenal[ai].monster === m.key) { aw = _arsenal[ai]; break; } }
  ...
  function pwsBeltPropose_() {
    ...
    function takeArsenal(w) { if (!w) return; if (seen[w.key] || picked.length >= window.Belt.max) return; seen[w.key] = 1; picked.push(pwsArsenalItem_(w)); }
    try { var wk0 = (_armory && _armory.week) || []; for (var a0 = 0; a0 < wk0.length; a0++) { for (var a1 = 0; a1 < _arsenal.length; a1++) { if (_arsenal[a1].monster === wk0[a0].key) takeArsenal(_arsenal[a1]); } } } catch (eA0) {}
    for (var a2 = 0; a2 < _arsenal.length; a2++) { takeArsenal(_arsenal[a2]); }
  ...
  /* v91_38 THE MONSTER: hang the tiles on four shelves, one open at a time */
  var _shelfOpen = '';
  function pwsHangShelves_(container) {
    var cards = Array.prototype.slice.call(container.querySelectorAll('.pws-tool-card[data-tool-idx], .pws-tool-card[data-group]'));
    if (!cards.length) return;
    var tools = (_toolsData && _toolsData.confirmedTools) ? _toolsData.confirmedTools : [];
    var byKey = { pile: [], fog: [], cold: [], conq: [] };
    cards.forEach(function(c) {
      var idx = parseInt(c.getAttribute('data-tool-idx'), 10), t = isNaN(idx) ? null : tools[idx];
      var k = t ? pwsMonsterOfTool_(t) : 'pile';
      if (!t && c.getAttribute('data-group')) k = 'fog';   /* a Talent project group hangs with the Fog */
      byKey[k].push(c);
    });
    /* v91_41: the Arsenal's shipped weapons hang on the shelf of the monster their law fights */
    _arsenal.forEach(function(w) {
      ...
      var kd = document.createElement('div'); kd.className = 'pws-weapon-kind'; kd.textContent = w.kind === 'inherited' ? (_lang === 'es' ? 'DE LA CASA' : 'THE HOUSE\u2019S') : (_lang === 'es' ? 'DE ACE' : 'ACE\u2019S');
      ...
      byKey[w.monster].push(wc);
    });
    var week = (_armory && _armory.week) ? _armory.week : [];
    if (!_shelfOpen) { _shelfOpen = week.length ? week[0].key : 'pile'; if (!byKey[_shelfOpen].length) { for (var i0 = 0; i0 < MONSTER_KEYS.length; i0++) { if (byKey[MONSTER_KEYS[i0]].length) { _shelfOpen = MONSTER_KEYS[i0]; break; } } } }
    var order = week.map(function(w) { return w.key; }); MONSTER_KEYS.forEach(function(k) { if (order.indexOf(k) === -1) order.push(k); });
    var es = _lang === 'es', imgs = window.PWS_ARMORY_IMAGES || {};
    order.forEach(function(k) {
      var shelf = document.createElement('div'); shelf.className = 'pws-shelf' + (k === _shelfOpen ? ' open' : ''); shelf.setAttribute('data-shelf', k);
      var head = document.createElement('div'); head.className = 'pws-shelf-head';
      if (imgs[k]) head.style.backgroundImage = 'url(' + JSON.stringify(imgs[k]) + ')';
      var cnt = document.createElement('div'); cnt.className = 'pws-shelf-count'; cnt.textContent = byKey[k].length + (es ? (byKey[k].length === 1 ? ' HERRAMIENTA' : ' HERRAMIENTAS') : (byKey[k].length === 1 ? ' TOOL' : ' TOOLS'));
      var ttl = document.createElement('div'); ttl.className = 'pws-shelf-title';
      var nm = document.createElement('span'); nm.textContent = MONSTER_NAME[k][es ? 'es' : 'en'];
      var hd = document.createElement('span'); hd.className = 'pws-shelf-heads';
      var heads = (_armory && _armory.kills) ? (_armory.kills[k] || 0) : null;
      var wk = null; for (var wi = 0; wi < week.length; wi++) { if (week[wi].key === k) wk = week[wi]; }
      hd.textContent = (heads !== null ? (heads + (es ? (heads === 1 ? ' CABEZA' : ' CABEZAS') : (heads === 1 ? ' HEAD' : ' HEADS'))) : '') + (wk ? ('  \u00b7  ' + (es ? 'ESTA SEMANA' : 'THIS WEEK')) : '');
      ttl.appendChild(nm); ttl.appendChild(hd); head.appendChild(cnt); head.appendChild(ttl);
      var body = document.createElement('div'); body.className = 'pws-shelf-body';
      byKey[k].forEach(function(c) { body.appendChild(c); });
      head.addEventListener('click', function() { var was = shelf.classList.contains('open'); Array.prototype.forEach.call(container.querySelectorAll('.pws-shelf'), function(sh) { sh.classList.remove('open'); }); if (!was) { shelf.classList.add('open'); _shelfOpen = k; } else { _shelfOpen = ''; } });
      shelf.appendChild(head); shelf.appendChild(body);
