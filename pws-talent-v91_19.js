...
(function() {
  ...
  */
  ...
  function pwsExtractText_(file, base64) {
    return post({ action: 'pwsExtractText', sessionId: _session, requestingMemberId: _memberId,
      ...
      .then(function(d) {
        ...
        else m = _lang === 'es' ? 'No pude leer ese archivo. Intenta de nuevo.' : 'I couldn\u2019t read that file. Try again in a moment.';
        throw new Error(m);
      }, function() {
  ...
  function pwsRenderUseFooter(isTimer, isMedia, savedLink) {
    ...
    if (window.SuperBucket) {
      ...
      window.SuperBucket.mount(useBucketHost, {
        ...
        askTitle: false,
        onFile: function(file) { pwsUseFileToDocB_(file); }
      });
  ...
  function pwsUseFileToDocB_(file) {
    if (!file) return;
    if (_useSending) { pwsViewToast_(_lang === 'es' ? 'Doc B sigue respondiendo \u2014 su\u00e9ltalo en un momento' : 'Doc B is still answering \u2014 drop it in a moment'); return; }
  ...
  window.pwsSendUse = function pwsSendUse() {
    ...
    if (fileSend) {
      ...
      userContent = [ fileSend.block, { type: 'text', text: text || displayText } ];
    ...
    // -------------------------------------------------------
    // -------------------------------------------------------
    if (!fileSend && _useAwaitingDread && _useTool) {
    ...
    if (!fileSend && _useAwaitingReframe && _useTool) {
    ...
    function useSendRelease_() { _useSending = false; if (useSendBtn) { useSendBtn.disabled = false; useSendBtn.style.opacity = ''; } }
