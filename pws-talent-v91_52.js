  ...
  */
  ...
  (function pwsArmoryCss_() {
    ...
    st.appendChild(document.createTextNode(
      ...
      '#pwsBeltRow .pws-belt-whisper{grid-column:1 / -1;text-align:center;font-family:\'Cormorant Garamond\',serif;font-style:italic;font-size:18px;color:#fff3c4;text-shadow:0 2px 8px rgba(0,0,0,1);margin-top:8px;opacity:0;transition:opacity .35s ease;pointer-events:none;}' +
      ...
      '.pws-rack-line{font-family:Cinzel,serif;font-size:12px;letter-spacing:0.16em;color:#c8a84b;margin:14px 0 0;display:flex;justify-content:space-between;}' +
      ...
      '.pws-ec-rooms{display:flex;align-items:center;justify-content:space-evenly;width:100%;background:#040608;border:1px solid rgba(200,168,75,0.35);border-top:none;border-radius:0 0 10px 10px;margin-top:-2px;}' +
      '.pws-ec-room{font-family:Cinzel,serif !important;font-size:14px !important;font-weight:600 !important;letter-spacing:0.22em !important;text-transform:uppercase !important;text-decoration:none !important;color:rgba(240,230,204,0.65) !important;white-space:nowrap;flex:1;height:56px;display:flex;align-items:center;justify-content:center;border-bottom:3px solid transparent;transition:color .2s,border-color .2s;}' +
      '.pws-ec-room:hover{color:#f0e6cc !important;border-bottom-color:#c8a84b;}' +
      '@media (max-width:600px){.pws-ec-rooms{display:grid;grid-template-columns:repeat(3,1fr);}.pws-ec-room{height:52px;font-size:12px !important;letter-spacing:0.16em !important;}}' +
  ...
  (function wireBrunoDock_() {
    function tryWire() {
      ...
      if (!card) {
        ...
        card.appendChild(sub); card.appendChild(ttl);
  ...
  function pwsRenderToolsTile() {
    ...
    [['/games', 'GAMES', 'JUEGOS'], ['/music', 'MUSIC', 'M\u00daSICA'], ['/social', 'SOCIAL', 'SOCIAL'], ['/sports', 'SPORTS', 'DEPORTES'], ['/movies', 'MOVIES', 'PEL\u00cdCULAS'], ['/feed', 'NEWS', 'NOTICIAS']].forEach(function(r) {
      var a = document.createElement('a'); a.className = 'pws-ec-room'; a.href = r[0]; a.textContent = _lang === 'es' ? r[2] : r[1]; rooms.appendChild(a);
    });
    showtime.appendChild(rooms);
  ...
  function pwsHangShelves_(container) {
    ...
    var armoryAcc = acc(es ? 'LA ARMER\u00cdA' : 'THE ARMORY', weapons.length + (es ? ' ARMAS' : ' WEAPONS'), true);
    armoryAcc.body.appendChild(rowEl); armoryAcc.body.appendChild(rackLine); armoryAcc.body.appendChild(rack); armoryAcc.body.appendChild(panel);
  ...
  function pwsRenderUseFooter(isTimer, isMedia, savedLink) {
    ...
    (function pwsRemoveDoor_() {
      ...
      (function pwsFightsPicker_() {
        ...
        MONSTER_KEYS.forEach(function(k) {
          var b = document.createElement('button'); b.className = 'pws-use-footer-btn' + (cur.indexOf(k) !== -1 ? ' on' : ''); b.textContent = MONSTER_NAME[k][_lang === 'es' ? 'es' : 'en'];
          b.addEventListener('click', function(e) { e.stopPropagation(); var on = b.classList.toggle('on'); var next = []; Array.prototype.forEach.call(grid.querySelectorAll('.pws-use-footer-btn.on'), function(x) { next.push(x.getAttribute('data-k')); }); if (!next.length) { b.classList.add('on'); return; } pwsFightsSet_(nm, next); });
