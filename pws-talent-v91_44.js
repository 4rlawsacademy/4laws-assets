...
(function() {
  ...
  */
    ...
    post({ action: 'armoryGetRequests', sessionId: _session, requestingMemberId: _memberId }).then(function(d) {
      ...
      if (d && d.status === 'ok' && Array.isArray(d.requests)) {
        d.requests.forEach(function(r) {
          ...
          out.push({ key: 'arsenal:' + r.requestId, name: String(r.title || '').substring(0, 60), image: (r.imageUrl && String(r.imageUrl).indexOf('https://') === 0) ? r.imageUrl : '', url: r.shippedLink, law: law, monster: mk, kind: r.kind || 'commission' });
        });
      }
  ...
  function pwsHangShelves_(container) {
    ...
    var tools = (_toolsData && _toolsData.confirmedTools) ? _toolsData.confirmedTools : [];
    var byKey = { pile: [], fog: [], cold: [], conq: [] }, mine = { pile: [], fog: [], cold: [], conq: [] }, es = _lang === 'es', imgs = window.PWS_ARMORY_IMAGES || {};
    var allCards = [];   /* every tool, for ALL MY TOOLS */
    /* the member's own tools that are weapons -- on every shelf they fight on; the routine stays off */
    var daily = [];
    cards.forEach(function(c) {
      var idx = parseInt(c.getAttribute('data-tool-idx'), 10), t = isNaN(idx) ? null : tools[idx]; if (!t) return;
      c.parentNode && c.parentNode.removeChild(c);
      var ty = String(t.type || '').toLowerCase();
      /* v91_44: the daily side = the routine and the onboarding tools; the Armory side = forged tools (and whatever the member re-aims by hand) */
      var armorySide = (ty === 'forge') || !!(pwsFightsOverrides_()[pwsToolKey_(t).toLowerCase()]);
      if (!armorySide) { daily.push(c); return; }
      allCards.push(c.cloneNode(true));
      var fights = pwsFightsOf_(pwsToolKey_(t), t.law || t.lawTag || (t.isTalent ? 'talent' : ''));
      fights.forEach(function(k, i) { mine[k].push(i === 0 ? c : c.cloneNode(true)); });
    });
    ...
    /* every copy needs the same door */
    function wireCard(c) { if (!c.getAttribute('data-tool-idx') || c._wired) return; c._wired = true; var i2 = parseInt(c.getAttribute('data-tool-idx'), 10); var t2 = tools[i2]; c.addEventListener('click', function(ev) { if (ev.target.closest && ev.target.closest('button')) return; pwsOpenUse(i2, t2 && t2.configured === true); }); }
    MONSTER_KEYS.forEach(function(k) { mine[k].forEach(wireCard); }); allCards.forEach(wireCard);
    /* projects stand under their own heading beneath the wall */
    var projects = groups.map(function(g) { g.parentNode && g.parentNode.removeChild(g); return g; });
    /* the clones need the same door: wire every copy to the tool */
    MONSTER_KEYS.forEach(function(k) { byKey[k].forEach(function(c) { if (c.getAttribute('data-tool-idx') && !c._wired) { c._wired = true; var i2 = parseInt(c.getAttribute('data-tool-idx'), 10); var t2 = tools[i2]; c.addEventListener('click', function(ev) { if (ev.target.closest && ev.target.closest('button')) return; pwsOpenUse(i2, t2 && t2.configured === true); }); } }); });
    /* Ace's weapons and the house's page-weapon; the house's organs stay off the wall */
    var pageWeapons = _arsenal.filter(function(w) { return w.kind !== 'inherited'; }).map(function(w) { return { name: w.name, image: w.image, url: w.url, law: w.law, tag: es ? 'DE ACE' : 'ACE\u2019S' }; });
    pageWeapons.push({ name: es ? 'El Compa\u00f1ero (Doc B)' : 'The AI Companion', image: imgs.companion || '', url: '/ai-companion', law: 'respect', tag: es ? 'DE LA CASA' : 'THE HOUSE\u2019S', fightsKey: 'ai companion' });
    pageWeapons.forEach(function(w) {
      var fights = pwsFightsOf_(w.fightsKey || w.name, w.law);
      fights.forEach(function(k) {
        var wc = document.createElement('div'); wc.className = 'pws-tool-card pws-weapon-card' + (w.image ? '' : ' pws-weapon-plain');
        if (w.image) wc.style.backgroundImage = 'url(' + JSON.stringify(w.image) + ')';
        else { var mk = document.createElement('div'); mk.className = 'pws-weapon-mark'; mk.textContent = w.fightsKey === 'ai companion' ? 'DOC B' : '\u2694'; wc.appendChild(mk); }
        var kd = document.createElement('div'); kd.className = 'pws-weapon-kind'; kd.textContent = w.tag;
        var tt = document.createElement('div'); tt.className = 'pws-armory-card-title'; tt.textContent = '\u2694 ' + w.name;
        wc.appendChild(kd); wc.appendChild(tt);
        wc.addEventListener('click', function() { window.location.href = w.url; });
        byKey[k].push(wc);
      });
    });
    var week = (_armory && _armory.week) ? _armory.week : [];
    if (!_shelfOpen) { _shelfOpen = week.length ? week[0].key : 'pile'; }
    var order = MONSTER_KEYS.slice();
    var rowEl = document.createElement('div'); rowEl.className = 'pws-monsters';
    var openEl = document.createElement('div'); openEl.className = 'pws-shelf-open'; openEl.style.display = 'none';
    function openShelf(k) {
      _shelfOpen = k;
      Array.prototype.forEach.call(rowEl.querySelectorAll('.pws-monster'), function(m) { m.classList.toggle('open', m.getAttribute('data-shelf') === k); });
      openEl.innerHTML = '';
      if (!k) { openEl.style.display = 'none'; return; }
      var heads = (_armory && _armory.kills) ? (_armory.kills[k] || 0) : 0, wk = null; for (var wi = 0; wi < week.length; wi++) { if (week[wi].key === k) wk = week[wi]; }
      var line = document.createElement('div'); line.className = 'pws-shelf-line';
      var l1 = document.createElement('span'); l1.textContent = MONSTER_NAME[k][es ? 'es' : 'en'] + '  \u00b7  ' + byKey[k].length + (es ? (byKey[k].length === 1 ? ' ARMA' : ' ARMAS') : (byKey[k].length === 1 ? ' WEAPON' : ' WEAPONS'));
      var l2 = document.createElement('span'); l2.textContent = heads + (es ? (heads === 1 ? ' CABEZA' : ' CABEZAS') : (heads === 1 ? ' HEAD' : ' HEADS')) + (wk ? ('  \u00b7  ' + (es ? 'ESTA SEMANA' : 'THIS WEEK')) : '');
      line.appendChild(l1); line.appendChild(l2); openEl.appendChild(line);
      var body = document.createElement('div'); body.className = 'pws-shelf-body';
      if (!byKey[k].length && !mine[k].length) { var em = document.createElement('div'); em.className = 'pws-shelf-empty'; em.textContent = es ? 'Nada apunta a este monstruo todav\u00eda. Bruno puede forjar algo.' : 'Nothing aims at this monster yet. Bruno can forge one.'; body.appendChild(em); }
      if (byKey[k].length) { var s1 = document.createElement('div'); s1.className = 'pws-shelf-section'; s1.textContent = es ? 'ARMAS DE ACE' : 'ACE\u2019S WEAPONS'; body.appendChild(s1); byKey[k].forEach(function(c) { body.appendChild(c); }); }
      if (mine[k].length) { var s2 = document.createElement('div'); s2.className = 'pws-shelf-section'; s2.textContent = es ? 'TUS HERRAMIENTAS' : 'YOUR TOOLS'; body.appendChild(s2); mine[k].forEach(function(c) { body.appendChild(c); }); }
      openEl.appendChild(body); openEl.style.display = 'block';
    }
    function openAll() {
      _wallView = 'all';
      Array.prototype.forEach.call(rowEl.querySelectorAll('.pws-monster'), function(m) { m.classList.remove('open'); });
      openEl.innerHTML = '';
      var line = document.createElement('div'); line.className = 'pws-shelf-line';
      var l1 = document.createElement('span'); l1.textContent = (es ? 'TODAS MIS HERRAMIENTAS  \u00b7  ' : 'ALL MY TOOLS  \u00b7  ') + allCards.length; line.appendChild(l1); openEl.appendChild(line);
      var body = document.createElement('div'); body.className = 'pws-shelf-body';
      allCards.forEach(function(c) { body.appendChild(c); });
      openEl.appendChild(body); openEl.style.display = 'block';
    }
    order.forEach(function(k) {
      ...
      m.addEventListener('click', function() { openShelf(k); });
      rowEl.appendChild(m);
    });
    var tabs = document.createElement('div'); tabs.className = 'pws-wall-tabs';
    var allBtn = document.createElement('button'); allBtn.type = 'button'; allBtn.textContent = es ? 'TODAS MIS HERRAMIENTAS' : 'ALL MY TOOLS'; allBtn.className = _wallView === 'all' ? 'on' : '';
    allBtn.addEventListener('click', function() { allBtn.classList.add('on'); openAll(); });
    tabs.appendChild(allBtn);
    /* v91_44 TWO ACCORDIONS */
    ...
    if (!daily.length) { var de = document.createElement('div'); de.className = 'pws-shelf-empty'; de.textContent = es ? 'Nada todav\u00eda.' : 'Nothing yet.'; dailyAcc.body.appendChild(de); }
    var armoryAcc = acc(es ? 'LA ARMER\u00cdA' : 'THE ARMORY', (_arsenal.filter(function(w) { return w.kind !== 'inherited'; }).length + 1 + allCards.length) + (es ? ' ARMAS' : ' WEAPONS'), true);
    armoryAcc.body.appendChild(rowEl); armoryAcc.body.appendChild(openEl);
    if (projects.length) { var pj = document.createElement('div'); pj.className = 'pws-projects'; var ph = document.createElement('div'); ph.className = 'pws-shelf-section'; ph.textContent = es ? 'PROYECTOS' : 'PROJECTS'; pj.appendChild(ph); var pg = document.createElement('div'); pg.className = 'pws-shelf-body'; projects.forEach(function(g) { pg.appendChild(g); }); pj.appendChild(pg); armoryAcc.body.appendChild(pj); }
    container.appendChild(dailyAcc.el); container.appendChild(armoryAcc.el);
    var inh = document.createElement('div'); inh.className = 'pws-inherit-line';
    inh.innerHTML = (es ? 'Cada sala lleva las armas propias de la casa \u2014 ' : 'Every room carries the house\u2019s own weapons \u2014 ') + '<a href="/arsenal">' + (es ? 'v\u00e9alas en el Arsenal' : 'see them at the Arsenal') + '</a>.';
    container.appendChild(inh);
    openShelf(_shelfOpen || 'pile');
  }
