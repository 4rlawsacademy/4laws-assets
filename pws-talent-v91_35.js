...
(function() {
  ...
  */
  ...
  window.pwsOpenUse = function pwsOpenUse(toolIdx, fromLaunch) {
    ...
    if (fromLaunch && !tool.configured) {
      ...
      if (savedTurnsR.length) {
        ...
        openingMsg = filterPrefix + (_lang === 'es' ? 'Retomamos donde lo dejamos \u2014 tengo la \u00faltima charla y todo lo que guardaste en esta ventana. \u00bfPor d\u00f3nde seguimos?' : 'Picking up where we left off \u2014 I have last time\u2019s chat and everything you saved in this window. Where do we go from here?');
      }
  ...
  window.pwsSendUse = function pwsSendUse() {
    ...
    _useSending = true;
    var useSendBtn = document.querySelector('#pwsUseOverlay .pws-docb-send-btn');
    ...
    .then(function(d) {
      ...
      if (!reply) { _useBuildRequest = false; useAppendBubble(_lang === 'es' ? 'Intenta de nuevo.' : 'Try again in a moment.', 'docb'); return; }
