  ...
  */
  ...
  (function pwsArmoryCss_() {
    ...
    st.appendChild(document.createTextNode(
      ...
      '#pwsBeltRow .pws-belt-say{grid-column:1 / -1;text-align:center;font-family:\'Cormorant Garamond\',serif;font-style:italic;font-size:17px;color:#fff3c4;text-shadow:0 2px 8px rgba(0,0,0,1);margin-top:8px;display:none;}' +
      '#pwsBeltRow.arming .pws-belt-say{display:block;}' +
      '#pwsBeltRow .pws-belt-whisper{grid-column:1 / -1;text-align:center;font-family:Cinzel,serif;font-weight:700;font-size:24px;letter-spacing:0.06em;color:#fff3c4;text-shadow:0 2px 10px rgba(0,0,0,1),0 0 18px rgba(0,0,0,0.9);margin-top:12px;opacity:0;transition:opacity .35s ease;pointer-events:none;}' +
  ...
  function pwsDockPanel_() {
    var dock = document.getElementById('pwsBrunoDock'), panel = document.getElementById('pwsUsePanel');
  ...
  function pwsUndockPanel_() {
    ...
    var dock = document.getElementById('pwsBrunoDock'); if (dock) dock.classList.remove('folded');
  }
  function pwsPanelDocked_() { var d = document.getElementById('pwsBrunoDock'), p = document.getElementById('pwsUsePanel'); return !!(d && p && p.parentNode === d); }
  function pwsBrunoFold_(on) { var d = document.getElementById('pwsBrunoDock'); if (!d) return; if (on === undefined) on = !d.classList.contains('folded'); d.classList.toggle('folded', !!on); }
  ...
  window.pwsBrunoOpen_ = function pwsBrunoOpen_(seed, quiet) {
    var dock = document.getElementById('pwsBrunoDock'); if (dock) { if (quiet) { dock.classList.add('folded'); } else { dock.classList.remove('folded'); } }
  ...
  (function wireBrunoDock_() {
    function tryWire() {
      ...
      dock.addEventListener('click', function(e) {
        ...
        if (x && pwsPanelDocked_()) { e.preventDefault(); e.stopImmediatePropagation(); pwsBrunoFold_(true); }
      }, true);
    }
    tryWire();
  })();
  ...
  function pwsHangShelves_(container) {
    ...
    function acc(title, n, open, memKey) {
      ...
      a.className = 'pws-acc' + (open ? ' open' : ''); var hd = document.createElement('div'); hd.className = 'pws-acc-head'; var t1 = document.createElement('span'); t1.textContent = title; var t2 = document.createElement('span'); t2.className = 'pws-acc-n'; t2.textContent = n; hd.appendChild(t1); hd.appendChild(t2); var bd = document.createElement('div'); bd.className = 'pws-acc-body';
      hd.addEventListener('click', function() { var nowOpen = a.classList.toggle('open'); try { localStorage.setItem('4laws-fold-' + memKey, nowOpen ? '0' : '1'); } catch (eS) {} });
      a.appendChild(hd); a.appendChild(bd); return { el: a, body: bd };
    }
  ...
  window.pwsOpenUse = function pwsOpenUse(toolIdx, fromLaunch) {
    ...
    if (tool.isBruno) { pwsDockPanel_(); }
    else { if (_useTool && _useTool.isBruno) { _useSwitching = true; try { pwsCloseUse(); } catch (eSw) {} _useSwitching = false; } pwsUndockPanel_(); }
    var titleEl = document.getElementById('pwsUseTitle'); if (titleEl) titleEl.textContent = tool.isBruno ? 'BRUNO' : (oblLabel || toolName);
    var feed = document.getElementById('pwsUseFeed'); if (feed) feed.innerHTML = '';
    ...
    var ov = document.getElementById('pwsUseOverlay'); if (ov && !tool.isBruno) ov.classList.add('open');   /* v91_36: the docked door needs no overlay */
    /* v91.2 THE REACHABLE DOOR: constrain the panel, lock the world. */
    try {
      if (!tool.isBruno) document.body.style.overflow = 'hidden';   /* v91_36: the docked door never freezes the page */
      if (ov) {
        var pnl92 = ov.querySelector('.pws-use-panel') || ov.firstElementChild;
        if (pnl92) {
          pnl92.style.maxHeight = '92vh';
    ...
    if (fromLaunch && !tool.configured) {
      ...
      if (tool.isBruno && savedTurnsR.length > 6) { savedTurnsR = savedTurnsR.slice(-6); }   /* v91_36: a forge remembers the last forgings, not a life */
      if (savedTurnsR.length) {
        ...
        if (!tool.isBruno) openingMsg = filterPrefix + (_lang === 'es' ? 'Retomamos donde lo dejamos \u2014 tengo la \u00faltima charla y todo lo que guardaste en esta ventana. \u00bfPor d\u00f3nde seguimos?' : 'Picking up where we left off \u2014 I have last time\u2019s chat and everything you saved in this window. Where do we go from here?');
      }
  ...
  function pwsRenderUseFooter(isTimer, isMedia, savedLink) {
    ...
    if (pwsBrunoIs_()) { try { var fsb = footer.querySelector('#pwsForgeStBtn'); if (fsb) fsb.style.display = 'none'; } catch (eFs) {} }
    /* v91_36 THE FULL CARRIAGE: what a todo's room has and this one lacked */
    if (!pwsBrunoIs_() && !(_useTool && (_useTool.isTalent === true || _useTool.type === 'talent'))) {
  ...
  window.pwsCloseUse = function pwsCloseUse() {
    ...
    if (!_useSwitching) { setTimeout(function() { try { pwsBrunoOpen_('', true); } catch (eRe) {} }, 0); }   /* v91_37: lands home as the closed card */
    void wasBruno;
  };
