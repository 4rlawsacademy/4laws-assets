  ...
  */
  ...
  (function pwsArmoryCss_() {
    ...
    st.appendChild(document.createTextNode(
      ...
      '#pwsBeltRow .pws-belt-title{font-family:Cinzel,serif;font-size:12px;letter-spacing:0.18em;color:#c8a84b;margin-bottom:8px;display:flex;justify-content:space-between;align-items:baseline;}' +
      '#pwsBeltRow .pws-belt-note{font-family:\'Cormorant Garamond\',serif;font-style:italic;font-size:14px;color:rgba(240,230,204,0.65);letter-spacing:0;text-transform:none;}' +
      '#pwsBeltRow .pws-belt-tiles{display:flex;flex-wrap:wrap;gap:8px;}' +
      ...
      '.pws-armory-doors{display:flex;gap:10px;justify-content:flex-end;align-items:center;flex-wrap:wrap;margin:0 0 12px;}' +
  ...
  function pwsBeltRender_() {
    ...
    if (!row.tools.length) { var em = document.createElement('div'); em.className = 'pws-belt-empty'; em.textContent = es ? 'Vac\u00edo. El cintur\u00f3n es para armas, no para la rutina: abre una y toca PONER EN EL CINTUR\u00d3N.' : 'Empty. The belt is for weapons, not the routine: open one and tap PUT ON THE BELT.'; rowEl.appendChild(em); return; }
    var tiles = document.createElement('div'); tiles.className = 'pws-belt-tiles';
    row.tools.forEach(function (it) {
      var b = document.createElement('button'); b.type = 'button'; b.className = 'pws-belt-tile' + ((row.pins || []).indexOf(it.key) !== -1 ? ' pinned' : '');
      b.textContent = it.name;
      if (it.image) { b.style.cssText = 'background:linear-gradient(rgba(4,6,8,0.25),rgba(4,6,8,0.85)),url(' + JSON.stringify(it.image) + ') center/cover no-repeat;min-height:64px;text-shadow:0 2px 6px rgba(0,0,0,0.9);'; }
      b.addEventListener('click', function () { if (it.kind === 'page' && it.url) { window.location.href = it.url; return; } if (!pwsBeltOpenTool_(it.key)) { try { window.Belt.say(es ? 'Esa herramienta ya no cuelga en tu pared.' : 'That tool no longer hangs on your wall.'); } catch (e) {} } });
      tiles.appendChild(b);
    });
  ...
  function pwsHangShelves_(container) {
    ...
    container.appendChild(dailyAcc.el); container.appendChild(armoryAcc.el);
