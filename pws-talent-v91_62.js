  ...
  */
  ...
  (function pwsArmoryCss_() {
    ...
    st.appendChild(document.createTextNode(
      ...
      '.pws-order .pws-order-num{position:absolute;left:8px;top:8px;width:26px;height:26px;border-radius:50%;background:#f2d585;color:#040608;font-family:Cinzel,serif;font-weight:700;font-size:14px;line-height:26px;text-align:center;box-shadow:0 2px 8px rgba(0,0,0,0.8);}' +
      '.pws-order .pws-order-word{position:absolute;right:8px;top:8px;font-family:Cinzel,serif;font-weight:700;font-size:11px;letter-spacing:0.14em;color:#040608;background:#f2d585;border-radius:12px;padding:4px 8px;}' +
  ...
  function pwsBeltRender_() {
    ...
    rowEl.innerHTML = '';
    var es = _lang === 'es', row = (window.Belt && window.Belt.row()) || { tools: [], pins: [], proposed: false };
    ...
    var t1 = document.createElement('span'); t1.textContent = es ? 'EN TU CINTUR\u00d3N' : 'ON YOUR BELT';
    ...
    for (var si = 0; si < maxN; si++) {
      ...
      (function(slot, item, btn) {
        ...
        btn.addEventListener('drop', function(e) { e.preventDefault(); btn.classList.remove('over'); var key = ''; try { key = e.dataTransfer.getData('text/plain'); } catch (eD) {} if (key) pwsBeltSeat_(key, slot); });
        if (item) {
          ...
          x.addEventListener('click', function(e) { e.stopPropagation(); e.preventDefault(); window.Belt.remove(item.key).then(function() { pwsBeltRender_(); }); });
          btn.appendChild(x);
          var holdT = null;
  ...
  function pwsBeltSeat_(key, slot) {
    if (!window.Belt) return;
    var fromBelt = key.indexOf('belt:') === 0; if (fromBelt) key = key.slice(5);
    var item = pwsBeltItemFor_(key); if (!item) return;
    var row = JSON.parse(JSON.stringify(window.Belt.row() || { tools: [], pins: [], proposed: false }));
    ...
    tools[slot] = item;
    row.tools = tools.filter(function(t) { return !!t; }); row.proposed = false;
    window.Belt.save(row).then(function() { pwsBeltRender_(); try { window.Belt.say(_lang === 'es' ? '\u2694 En tu cintur\u00f3n: ' + item.name : '\u2694 On your belt: ' + item.name); } catch (eS) {} });
  }
  ...
  window.BeltHost = {
    name: 'talent',
    fire: function (item) { if (!item) return false; if (item.kind === 'page' && item.url) { window.location.href = item.url; return true; } if (item.kind !== 'tool') return false; return pwsBeltOpenTool_(item.key); },
    drop: function (files) {
  ...
  function pwsRespectRow_() { var c = (_windowConfig && _windowConfig['respect']) ? _windowConfig['respect'] : null; if (typeof c === 'string') { try { c = JSON.parse(c); } catch (e) { c = null; } } if (!c || typeof c !== 'object') c = {}; if (!c.rels) c.rels = {}; if (!c.adjustments) c.adjustments = []; if (!c.games) c.games = []; return c; }
  ...
  function pwsRespectGreeting_(tool) {
    ...
    if (tool.respectKind === 'rel') {
      ...
      if (rec && rec.at) { return es ? ('La \u00faltima vez hablamos de ' + rel.who.es + ' hace ' + pwsDaysAgo_(rec.at) + ' d\u00edas. ' + (rec.goals && rec.goals.length ? ('Quedamos en: ' + rec.goals.join(' \u00b7 ') + '. ') : '') + '\u00bfAlgo cambi\u00f3?') : ('Last time we talked about ' + rel.who.en + ' was ' + pwsDaysAgo_(rec.at) + ' days ago. ' + (rec.goals && rec.goals.length ? ('We set: ' + rec.goals.join(' \u00b7 ') + '. ') : '') + 'Has something changed?'); }
  ...
  function pwsRespectPrompt_(tool) {
    ...
    if (tool.respectKind === 'rel') {
      ...
      return base + 'THIS ROOM is about ' + rel.who.en + ' (position ' + (RESPECT_RELS.indexOf(rel) + 1) + ' in the order). '
        + (rec && rec.at ? ('LAST INTERVIEW (' + rec.at.slice(0, 10) + '): answers ' + JSON.stringify(rec.answers || {}) + ', severities ' + JSON.stringify(rec.severity || {}) + ', goals ' + JSON.stringify(rec.goals || []) + '. If the member says something changed, interview again; otherwise work on the goals. ') : '')
        ...
        + 'THE GOALS: after the six, name ONE or TWO goals for this relationship in plain words, say them, and end your reply with this exact final line and nothing after it: '
        + 'RESPECT_GOALS {\"rel\":\"' + tool.respectRel + '\",\"answers\":{\"place\":\"yes|no\",\"returned\":\"yes|no\",\"balance\":\"yes|no\",\"tone\":\"yes|no\",\"control\":\"yes|no\",\"invisibility\":\"yes|no\"},\"severity\":{\"<question>\":1-5},\"goals\":[\"...\",\"...\"]} '
    ...
    /* the door */
    var summary = RESPECT_RELS.map(function(r) { var rec = row.rels[r.key]; return r.en + ': ' + (rec && rec.at ? ('talked ' + pwsDaysAgo_(rec.at) + ' days ago, goals ' + JSON.stringify(rec.goals || [])) : 'never'); }).join(' | ');
  ...
  function pwsRespectHarvest_(reply) {
    ...
    if (m) { try { var g = JSON.parse(m[1]); var k = g.rel || _useTool.respectRel; if (k) { row.rels[k] = { at: new Date().toISOString(), answers: g.answers || {}, severity: g.severity || {}, goals: Array.isArray(g.goals) ? g.goals.slice(0, 2) : [] }; pwsRespectSave_(row); setTimeout(function() { useAppendBubble((es ? '\u2713 Guardado en esta sala: ' : '\u2713 Kept in this room: ') + (row.rels[k].goals.join(' \u00b7 ') || (es ? 'la entrevista' : 'the interview')), 'docb'); }, 60); } } catch (e1) {} reply = reply.replace(/RESPECT_GOALS\s*\{[\s\S]*?\}\s*$/, '').replace(/\s+$/, ''); }
  ...
  function pwsRespectRender_() {
    ...
    var mh = document.createElement('div'); mh.className = 'pws-armory-title'; mh.style.cursor = 'default'; var m1 = document.createElement('span'); m1.textContent = es ? 'MONSTRUOS' : 'MONSTERS'; var m2 = document.createElement('span'); m2.className = 'pws-acc-n'; m2.textContent = es ? 'TOCA UNO' : 'TAP ONE'; mh.appendChild(m1); mh.appendChild(m2); host.appendChild(mh);
  ...
  function pwsHangShelves_(container) {
    ...
    function drawRack(filterKey) {
      ...
      var list = weapons.filter(function(w) { return !filterKey || w.fights.indexOf(filterKey) !== -1; });
      rackLine.innerHTML = '';
    ...
    function acc(title, n, open, memKey) {
      ...
      a.className = 'pws-acc'; var hd = document.createElement('div'); hd.className = 'pws-acc-head'; var t1 = document.createElement('span'); t1.textContent = title; var t2 = document.createElement('span'); t2.className = 'pws-acc-n'; t2.textContent = n; hd.appendChild(t1); hd.appendChild(t2); var bd = document.createElement('div'); bd.className = 'pws-acc-body';
    ...
    var armoryAcc = acc(es ? 'MONSTRUOS' : 'MONSTERS', es ? 'TOCA UNO' : 'TAP ONE', true, 'monsters');
    ...
    function showAll() { Array.prototype.forEach.call(rowEl.querySelectorAll('.pws-monster'), function(x) { x.classList.remove('open'); }); _shelfOpen = ''; drawRack(''); try { armoryTitle.scrollIntoView({ block: 'start', behavior: 'smooth' }); } catch (eS) {} }
    armoryTitle.addEventListener('click', function(e) { e.stopPropagation(); showAll(); });
    armoryAcc.body.appendChild(rowEl); armoryAcc.body.appendChild(armoryTitle); armoryAcc.body.appendChild(rackLine); armoryAcc.body.appendChild(rack); armoryAcc.body.appendChild(panel);
  ...
  window.pwsSendUse = function pwsSendUse() {
    ...
    if (_useTool && _useTool.isRespect) { systemPrompt = imgCapability + pwsRespectPrompt_(_useTool); }   /* v91_59 THE ORDER OF IMPORTANCE */
