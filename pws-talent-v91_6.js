/* BENCH 26 pws-talent v91_6 — THE TRUE TIMER: adds a real countdown
...
(function() {
  ...
  */
  ...
  function pwsViewEnsureBtn_() {
    ...
    if (!document.getElementById('pwsCdToggle')) {
      var cdBtn = document.createElement('button');
      cdBtn.className = 'pws-voice-btn';
      cdBtn.id = 'pwsCdToggle';
      cdBtn.title = 'Timer';
      cdBtn.innerHTML = '\u23F0';
      cdBtn.addEventListener('click', pwsCdTogglePanel_);
      vBtn.parentNode.insertBefore(cdBtn, vBtn);
    }
  }
  ...
  function pwsCdTick_() {
    ...
    var btn = document.getElementById('pwsCdToggle');
    if (btn) { btn.innerHTML = left > 0 ? '\u23F3' : '\u23F0'; }
    if (left <= 0) { pwsCdFinish_(true); }
  }
  function pwsCdFinish_(ring) {
    if (_cdInterval) { clearInterval(_cdInterval); _cdInterval = null; }
    _cdEndAt = 0; pwsCdSaveKey_();
    var btn = document.getElementById('pwsCdToggle'); if (btn) { btn.innerHTML = '\u23F0'; }
    if (ring) { pwsCdRing_(); }
