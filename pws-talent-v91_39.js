...
(function() {
  ...
  */
  ...
  function pwsWeaponFor_(key) {
    ...
    var best = null;
    for (var i = 0; i < tools.length; i++) { var t = tools[i]; if (pwsMonsterOfTool_(t) !== key) continue; if (!best || (t.configured === true && best.configured !== true)) best = t; }
    return best;
  }
  ...
  function pwsBeltPropose_() {
    ...
    var row = window.Belt.row(); if (!row || row.tools.length) return;
    var tools = (_toolsData && _toolsData.confirmedTools) ? _toolsData.confirmedTools.filter(pwsIsValidTool) : [];
    if (!tools.length) return;
    var picked = [], seen = {};
    function take(t) { if (!t) return; var k = pwsToolKey_(t); if (!k || seen[k] || picked.length >= window.Belt.max) return; seen[k] = 1; picked.push(pwsBeltItem_(t)); }
    /* today's schedule first */
    try { var sched = (_dayData && _dayData.finalSchedule) || []; for (var i = 0; i < sched.length; i++) { var lbl = String(sched[i].label || ''); for (var j = 0; j < tools.length; j++) { if (pwsToolKey_(tools[j]) === lbl) take(tools[j]); } } } catch (e1) {}
    ...
    window.Belt.propose(picked).then(function () { pwsBeltRender_(); try { window.Belt.say(_lang === 'es' ? 'Te propuse un cinturón. Cámbialo como quieras.' : 'I proposed a belt for you. Change it as you like.'); } catch (eS) {} });
  }
  function pwsBeltRender_() {
    ...
    if (!row.tools.length) { var em = document.createElement('div'); em.className = 'pws-belt-empty'; em.textContent = es ? 'Vac\u00edo. Abre una herramienta y toca PONER EN EL CINTUR\u00d3N.' : 'Empty. Open a tool and tap PUT ON THE BELT.'; rowEl.appendChild(em); return; }
    ...
    row.tools.forEach(function (it) {
      ...
      b.addEventListener('click', function () { if (!pwsBeltOpenTool_(it.key)) { try { window.Belt.say(es ? 'Esa herramienta ya no cuelga en tu pared.' : 'That tool no longer hangs on your wall.'); } catch (e) {} } });
      tiles.appendChild(b);
    });
  ...
  window.BeltHost = {
    name: 'talent',
    fire: function (item) { if (!item || item.kind !== 'tool') return false; return pwsBeltOpenTool_(item.key); },
    drop: function (files) {
  ...
  window.pwsBrunoBoot_ = function pwsBrunoBoot_() {
    try { if (!_useTool && _session && _memberId) { pwsArmoryFetch_(function() {
      pwsBrunoOpen_('', true);
